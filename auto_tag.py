...
# Updated to use Mistral's model natively based on your JS reference
...
# Mistral API Call
# ---------------------------------------------------------
async def _call_mistral_api(items: list[dict], batch_index: int) -> dict[str, list[str]]:
...
async def process_batch(batch: list[dict], semaphore: asyncio.Semaphore, batch_index: int) -> dict[str, list[str]]:
    ...
    if need_api:
        async with semaphore:
            api_results = await _call_mistral_api(need_api, batch_index)
